// SERVER-SIDE (Express) utilities
// lib/analytics.js

export function avgArrayPercentage (arr) {
      if (arr.length === 0) {
        return 0;
      }

      const sum = arr.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
      const avg = ((sum / arr.length) * 100).toFixed(0) + "%";
      return avg
    }
...
export function formatAgeRange({ minAge, maxAge }) {
  if (maxAge == null || Number.isNaN(Number(maxAge))) return `${minAge}+`;
